...
import { Home, Clock, MapPin, Calendar, Settings, LogOut, Moon } from 'lucide-react';
...
export default function Sidebar() {
  return (
    <aside className="flex flex-col items-center bg-[#181E2A] h-screen w-20 py-6 rounded-2xl m-2 shadow-lg">
      {/* Logo */}
      ...
      <nav className="flex flex-col gap-8 flex-1">
        {navItems.map((item, idx) => (
          <button key={idx} className="text-gray-400 hover:text-white transition-colors flex flex-col items-center group">
            {item.icon}
      ...
      <div className="flex flex-col gap-6 items-center mb-2">
        ...
        <button className="bg-[#232A3A] p-2 rounded-full text-gray-400 hover:text-white">
          <Moon size={20} />
        </button>
      </div>
    </aside>
